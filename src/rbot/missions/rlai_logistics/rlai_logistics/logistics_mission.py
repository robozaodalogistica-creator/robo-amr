...
LOCATIONS = {
    "home": (0.0, 0.0, 0.0),
    "pallet_1": (-2.0, 2.0, math.pi / 2.0),
    "pallet_2": (-2.0, -2.0, -math.pi / 2.0),
    "pallet_3": (2.0, 2.0, math.pi / 2.0),
    "pallet_4": (2.0, -2.0, -math.pi / 2.0),
    "expedicao": (4.0, -1.0, math.pi),
    "doca": (-4.0, -1.75, 0.0),
}
...
class LogisticsMission(Node):
    def __init__(self):
        ...
        for name, (x, y, yaw) in LOCATIONS.items():
    ...
    def _run_mission(self):
        ...
        delivered = 0
        for index, (pickup, delivery, label) in enumerate(TASKS, start=1):
            self.get_logger().info(f"[Task {index}/{len(TASKS)}] {label}")
        ...
        if delivered == len(TASKS):
            final = f"MISSION_SUCCESS delivered={delivered}/{len(TASKS)} t={elapsed:.1f}s"
            self.get_logger().info(final)
        else:
            final = f"MISSION_PARTIAL delivered={delivered}/{len(TASKS)} t={elapsed:.1f}s"
